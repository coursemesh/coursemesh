...
class ConfigTests(unittest.TestCase):
    def test_loads_local_source(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(loaded.sources[0].path, (root / "demo.ics").resolve())
    ...
    def test_rejects_state_directory_outside_config_directory(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw) / "project"
            root.mkdir()
            cfg = root / "coursemesh.toml"
            cfg.write_text('''[coursemesh]\nstate_dir="../state"\n[[sources]]\nid="demo"\npath="demo.ics"\n''')
            with self.assertRaisesRegex(ValueError, "state_dir.*within"):
                load_config(cfg)

    def test_rejects_output_calendar_outside_config_directory(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw) / "project"
            root.mkdir()
            cfg = root / "coursemesh.toml"
            cfg.write_text('''[coursemesh]\noutput_calendar="../calendar.ics"\n[[sources]]\nid="demo"\npath="demo.ics"\n''')
            with self.assertRaisesRegex(ValueError, "output_calendar.*within"):
                load_config(cfg)

    def test_rejects_local_source_outside_config_directory(self):
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw) / "project"
            root.mkdir()
            cfg = root / "coursemesh.toml"
            cfg.write_text('''[[sources]]\nid="demo"\npath="../private.ics"\n''')
            with self.assertRaisesRegex(ValueError, "Source 'demo' path.*within"):
                load_config(cfg)
